...
class ModifiedRubberBottomSheet extends StatefulWidget {

  const ModifiedRubberBottomSheet({Key key,
  ...
  final Function(bool) ondraglast;
...
class RubberBottomSheetState extends State<ModifiedRubberBottomSheet> with TickerProviderStateMixin, AfterLayoutMixin<ModifiedRubberBottomSheet> {
            ...
            if (details.velocity.pixelsPerSecond.dy.abs() > _kMinFlingVelocity) {
              if (controller.value > controller.halfBound) {
                controller.fling(controller.halfBound, controller.upperBound,
                    velocity: flingVelocity);
                    widget.ondraglast(true);
              } else {
                widget.ondraglast(false);
